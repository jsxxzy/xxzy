...
export default {
  ...
  created(){
    sessionStorage.setItem('thirdLogin',this.$route.query.type)
    //单点登录判断
  ...
  methods:{
    ...
    async login(){
      const data = {
        ...
        toMenu:this.$route.query.toMenu
      }
      const res = await Api.thirdLoginBind(data)
