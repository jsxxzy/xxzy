<template>
  <div>
    <Header title="教学计划" type='0' route='/homeIndex'></Header>
    <div class="teachingPlan">
      ...
      <div class="content" v-if="this.type == '0'">
        <div class="content-item" v-for="(item, index) in pageData" :key="index">
          <p>{{ item.courseName }}</p>
          <van-row>
            <van-col span="12">
              <ul>
                ...
                <li>学期时：{{item.Semester}}</li>
                <li>课程编码：{{item.courseCode}}</li>
...
// WEBPACK FOOTER //
// src/components/newPages/teacher/train/teachingPlan.vue