      ...
      <div class="in_box">
        <template v-if="this.list2.scorType == 'D'">
          <template v-for="(item, index) in scoreList">
            <p :key="index">
              修改后{{ item.projectName }}
              <input
                type="number"
                placeholder="请点击填写"
                min="0"
                v-model="calcIndexList[index]"
                @keyup="numLimit(index)"
              />
            </p>
          </template>
  ...
  methods: {
    ...
    numLimit(val) {
      if (this.calcIndexList[val] < 0) {
        ...
        this.$toast.fail("成绩不能大于100");
      }
      return this.calcIndexList[val];
    },
    scoreLimit() {
      if (this.curScore < 0) {
        this.curScore = Math.abs(this.curScore).toString();
      } else if (this.curScore > 100) {
        this.curScore = 100;
        this.$toast.fail("成绩不能大于100");
      }
    ...
    async place() {
      ...
      if (res.code == "0") {
        this.$toast(res.Msg);
        this.$router.replace("/grade/update/index");
      }
    ...
    fn1() {
      ...
      for (let i = 0; i < this.scoreList.length; i++) {
        ...
        total = total.concat(
          (
            Number(this.scoreList[i].proportion.slice(0, -1) / 100) *
            Number(this.proList[i])
          ).toFixed(1)
        );
      ...
      if (this.person == "" || this.valueRes == "") {
        this.$toast.fail("请填写修改原因或审核人");
      } else if (this.list2.scorType != "D" && this.valueGrade == "") {
        this.$toast.fail("请选择修改后的成绩");
      } else {
...
.modify-stu .van-picker__columns::before {
  width: 100px;
  height: 1px;
...
.modify-stu .van-picker__columns::after {
  width: 100px;
  height: 1px;
  background-color: @attendance-background-color-2;
  content: "";
...
.modify-stu .van-picker__confirm {
  color: rgba(0, 0, 0, 0);
...
.modify-stu .popNote i {
  width: 25px;
  height: 25px;
  padding: 16px;
}
.modify-stu .person button {
  border-radius: 3px;
  padding: 5px 15px;
  margin-right: 12px;
  margin-bottom: 15px;
...
.stu .top_boxs span:nth-child(2),
.top_boxs span:nth-child(3) {
  color: @attendance-color-3;
  font-size: 12px;
...
// WEBPACK FOOTER //
// src/components/newPages/teacher/grade/update/gradeUpdate.vue