<template>
  <div>
    <Header title="成绩修改" type="0" route="/homeIndex"></Header>
    <div class="grade">
      <van-tabs v-model="active" type="line" sticky animated>
        ...
        <van-tab title="修改记录">
          <div class="gradeList">
            ...
            <NOdata
              v-if="this.updateList && this.updateList.length == 0"
              title="未查询到成绩信息"
            ></NOdata>
            <div v-else class="list">
              <div
                class="list-item update"
                v-for="(item, index) of updateList"
                :key="index"
                @click="goUpdate(item)"
              >
                <p>{{ item.xm }}</p>
                <ul>
                  <li>学号：{{ item.xh }}</li>
                  <li>考试性质：{{ item.ksxzmc }}</li>
                </ul>
                <span :class="item.class">{{ item.shzt }} </span>
              </div>
            </div>
          </div>
        </van-tab>
...
export default {
  ...
  methods: {
    ...
    // 获取修改记录
    async getGradeUpdate() {
      const res = await Api.cjxgHisroty();
      this.list1 = res.data;

      for (let item of this.list1) {
        if (item.shzt == "审核中") {
          item.class = "shzClass";
        } else if (item.shzt == "不通过") {
          item.class = "btgClass";
        } else if (item.shzt == "通过") {
          item.class = "ytgClass";
          item.shzt = "已通过";
        } else if (item.shzt == "待送审") {
          item.class = "dssClass";
        }
      }
    }
  ...
  computed: {
    ...
    updateList: function() {
      var value = this.value;
      if (value) {
        return this.list1.filter(function(product) {
          return Object.keys(product).some(function(key) {
            return (
              String(product[key])
                .toLowerCase()
...
// WEBPACK FOOTER //
// src/components/newPages/teacher/grade/update/index.vue