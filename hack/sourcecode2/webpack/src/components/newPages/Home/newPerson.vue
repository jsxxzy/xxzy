<template>
  <div>
    ...
    <!-- </router-link> -->
    <router-link class="grey-contain" to="/forget" tag="div"  v-if="schoolCode =='12649'||schoolCode=='10534'">
      <p>忘记密码</p>
    </router-link>
    <!-- <router-link class="grey-contain" to="/untied" tag="div" v-if="isNeedLogout && isNeedLogout == 1">
      <p>修改密码</p>
    </router-link> -->
    <router-link class="grey-contain" to="/untied" tag="div" v-if="schoolCode =='10225'">
      <p>修改密码</p>
    </router-link>
    <!-- /untied -->
    ...
    <div class="grey-contain" @click="thirdLoginUnbindTap()" v-if="thirdLogin">
      <p>解绑</p>
      <i class="arrow"></i>
    </div>
  ...
  export default{
    data(){
      return{
        ...
        thirdLogin:sessionStorage.getItem('thirdLogin'),
    ...
    methods:{
      ...
      thirdLoginUnbindTap(){
        Dialog.confirm({
          ...
          this.thirdLoginUnbind()
        }).catch(() => {
          // on cancel
        });
      },
      async thirdLoginUnbind(){
        var bindType = sessionStorage.getItem('thirdLogin');
        const data = {
          type:bindType
        }
        const res = await Api.thirdLoginUnbind(data)
        if(res.data){
          this.$toast.success(res.Msg)
          sessionStorage.removeItem("Token");
          sessionStorage.removeItem("thirdLogin");
          location.href = res.data+"/login/"+bindType
        }
      }
    },
    components: { 
...
// WEBPACK FOOTER //
// src/components/newPages/Home/newPerson.vue