<template>
  <div>
    <Header title="更多" :casLogin="casLogin"></Header>
    <div class="classroom">
      <ul class="pick_list">
        <li>
          <div class="flex_item">
            <i class="icon_classroom"></i>
            <p>使用时间</p>
          </div>
        ...
        <li>
          <div class="flex_item">
            <i class="icon_classroom4"></i>
            <p>教室类型</p>
          </div>
          <div class="pick-input">
            <input
              type="text"
              @click="buildTypePicker = true"
              readonly
              v-model="value4"
              placeholder="非必选"
            />
        ...
        <van-popup v-model="buildTypePicker" position="bottom">
          <van-picker
            show-toolbar
            :columns="buildTypeList"
            @cancel="buildTypePicker = false"
            @confirm="buildTypeSelect"
            :item-height="itemHeight"
          />
        </van-popup>
      </ul>
      ...
      <ul class="kscx_menu">
        <li
          v-for="(item, index) in quickTime"
          :key="index"
          @click="pickQuik(item.code, item.time)"
        >
          {{ item.time }}
        </li>
      </ul>
...
export default {
  ...
  methods: {
    ...
    timeConfirm(res) {
      ...
      this.date =
        res.getFullYear() + "-" + (res.getMonth() + 1) + "-" + calDate;
    ...
    async getNode() {
      ...
      for (let i = 0; i < res.data.length; i++) {
        ...
        this.columns[0] = {
          className: "c1",
          values: concatList,
          defaultIndex: Number(this.$route.query.s_start) - 1
        };
        this.columns[1] = {
          className: "c1",
          values: concatList,
          defaultIndex: Number(this.$route.query.e_end) - 1
        };
      }
    },
    async getCampus() {
  ...
  created() {
    ...
    this.date = this.$route.query.d_date;
    this.start = this.$route.query.s_start;
    this.end = this.$route.query.e_end;
    this.value = this.start + "-" + this.end;
    this.value2=this.$route.query.xq.mc
    this.campusId=this.$route.query.xq.dm
    this.value3=this.$route.query.jxl.mc
    this.buildingId=this.$route.query.jxl.dm
    this.value4=this.$route.query.jslx.mc
    this.typeCode=this.$route.query.jslx.dm
  }
};
</script>
<style lang="less">
.classroom .van-popup.van-popup--bottom {
  border-radius: 5px 5px 0 0;
}
.classroom .van-action-sheet {
...
.classroom .el-input__inner {
  background: none;
  border: 0;
  text-align: right;
  padding-right: 40px;
}
.classroom .el-input--small .el-input__inner {
  padding: 0;
}
.classroom .pick_list {
  color: @attendance-color-1;
...
.classroom .pick_list li .flex_item > i {
  width: 25px;
  height: 25px;
  margin-right: 10px;
}
.classroom .pick_list li .icon_classroom {
  background: url(../../../../../static/img/class_a.png) no-repeat center;
  background-size: 25px 25px;
}
.classroom .pick_list li .icon_classroom4 {
  background: url("../../../../assets/images/icon_teacher2.png") no-repeat
    center;
  background-size: 25px 25px;
}
.classroom .pick-input > i {
  width: 25px;
  height: 13px;
}
.pick-arrow {
...
.classroom .pick_list li .icon_classroom1 {
  background: url(../../../../../static/img/class_b.png) no-repeat center;
  background-size: 25px 25px;
}
.classroom .pick_list li .icon_classroom2 {
  background: url(../../../../../static/img/class_c.png) no-repeat center;
  background-size: 25px 25px;
}
.classroom .pick_list li .icon_classroom3 {
  background: url(../../../../../static/img/class_d.png) no-repeat center;
  background-size: 25px 25px;
}
.classroom .kscx {
  text-align: center;
...
.classroom .el-input__prefix {
  left: 87%;
}
.classroom .el-icon-circle-close:before {
  content: "";
}
.pick_list li .pick-input {
  font-size: 16px;
  font-weight: 600;
  display: flex;
  align-items: center;
...
.pick_list li .pick-input > input {
  text-align: right;
  padding-right: 2px;
  width: 85%;
}
.pick_list li .pick-input > input::placeholder {
  color: @attendance-background-color-2;
  font-weight: 400;
}
.classroom
...
.classroom .van-picker__columns {
  position: relative;
}
.classroom .van-picker-column.column1::after {
  position: absolute;
  content: "到";
  width: auto;
  height: auto;
...
.classroom .van-picker-column.column2::after {
  position: absolute;
  content: "到";
  width: auto;
  height: auto;
...
.classroom
  .pick_list
  .date-pick-item.van-popup.van-popup--bottom
  .van-picker-column {
  position: relative;
}
.classroom
  ...
  .van-picker-column:first-child::after {
  position: absolute;
  content: "年";
  width: auto;
  height: auto;
...
.classroom
  ...
  .date-pick-item.van-popup.van-popup--bottom
  .van-picker.van-datetime-picker
  .van-picker-column:nth-child(2)::after {
  position: absolute;
  content: "月";
  width: auto;
  height: auto;
...
.classroom
  ...
  .date-pick-item.van-popup.van-popup--bottom
  .van-picker.van-datetime-picker
  .van-picker-column:nth-child(3)::after {
  position: absolute;
  content: "日";
  width: auto;
  height: auto;
...
// WEBPACK FOOTER //
// src/components/newPages/student/restRooms/roomMore.vue