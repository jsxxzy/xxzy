<template>
  <div class="my-applyRecord">
    <!-- <div class="title">
			<router-link to="classroom"><i></i>教室借用申请记录</router-link>
		</div> -->
    <Header title="学生教室借用"></Header>
    <NOdata title="暂无申请记录~" v-if="this.recordList.length == 0"></NOdata>
    <template v-else>
      <!-- @click="fn1(item)" -->

      <div class="record_box" v-for="(item, index) in recordList" :key="index">
        <div class="record_fl">
          <p>{{ item.courseName }}</p>
...
export default {
  ...
  methods: {
    async Record() {
      const res = await Api.myBorrowingClassrooms();
...
// WEBPACK FOOTER //
// src/components/newPages/student/roomBrrow/index.vue