  ...
  data(){
    return{
      ...
      data:[
        ...
        {"menu":"xs_xqkscj","path":"/student/semester_Score"},
        ...
        // 通知公告
        {"menu":"ysgg","path":"/adv"},
        {"menu":"dclxx","path":"/notice?type=0"},
        {"menu":"ydxx","path":"/notice?type=2"},
      ]
    }
  },
  created(){
    ...
    if(this.$route.query.thirdLogin){
      sessionStorage.setItem('thirdLogin',this.$route.query.thirdLogin)
    }else{
  ...
  methods:{
    codeDiffer(){
      ...
      if(this.$route.query.code == 1){
        ...
        this.response = "登录失败！"
      }else if(this.$route.query.code == 'wechatma'){
        this.response = "未绑定账号，请退出小程序重新进入！"
        this.isError = false
      }else if(this.$route.query.code == 'wechatmp'){
        this.response = "解绑成功，请关闭页面重新进入！"
        this.isError = false
      }else{
        this.response = "参数错误"
      }
    ...
    menuCheck(menu){
      let list = this.data.filter(item => item.menu == menu)
      if(list.length === 0){
        this.$router.replace('/homeIndex')
      }else{
        this.$router.replace(list[0].path)
      }
    },
    ifWechatBrowser(){
      let browser = {
