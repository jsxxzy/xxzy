<template>
  <div class="home pad-fix">
    ...
    <div class="home-items">
      ...
      <!-- 自定义菜单 -->
      <template v-for="item in freeMenu">
        <div
          class="icon-item"
          v-if="item.Name != '禁用'"
          :key="item.id"
      ...
      <!-- 不受权限控制临时菜单 -->
      <!-- <template v-for="item in uncontrol">
        <div
          class="icon-item"
          :key="item.id"
...
export default {
  data() {
    return {
      ...
      {
        icon: require('../../../static/img/ico16.png'),
        name: '补考查询',
      ...
      teacherList: [{
        ...
        icon: require('../../../static/img/ico18.png'),
        name: '我的课堂',
        path: '/teacher/create',
        code: 'js_wdkt',
      }],
  ...
  methods: {
    ...
    async gotMenu() {
      // sessionStorage.setItem('schoolCode','10548')
      // sessionStorage.setItem('schoolCode','10079')
      // sessionStorage.setItem('schoolCode','00000')
      // sessionStorage.setItem('schoolCode','10225')
      // sessionStorage.setItem('schoolCode','12561')
      // sessionStorage.setItem('schoolCode','8686')
      // sessionStorage.setItem('schoolCode','13104')
      // sessionStorage.setItem('schoolCode','11407')
      // sessionStorage.setItem('schoolCode','10759')
      // sessionStorage.setItem('schoolCode','4748')
    ...
    getPicMenu(){
      Api.getMenuAndPic().then((res) => {
        if(res.data.length == 0) return
        res.data.forEach(item => {
          item.codeIcon = "./static/img/" + item.icon + '.png'
        });
        this.freeMenu = res.data
      })
    },
    goUrl(url){
      if(url == ""){
  ...
  created() {
    ...
    this.gotMenu()
    // this.getPicMenu()
    this.$router.replace("/new/HomeBar");
  },
}
</script>
