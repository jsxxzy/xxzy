<template>
  <div class="teacher111">
    ...
    <div class="P_boxs">
      ...
      <p>
        时间：<span>{{ weekNote }}</span>
      </p>
      <p>
        地点：<span>{{ classroomName }}</span>
      </p>
      <p>
        <span class="fl_span">班级：</span
        ><span class="technology">{{ classNames }}</span>
      </p>
    </div>
    <div class="cho">
      <div class="my_spans">
      ...
      <div class="my_spans">
        <div class="flex_item">
          <img
            src="../../../assets/images/icon_people2.png"
            alt=""
            class="my_img"
          /><span>审核人</span>
        </div>
      ...
      <div class="my_spans">
        <div class="flex_item">
          <img
            src="../../../assets/images/icon_why2.png"
            alt=""
            class="my_img"
          /><span>调课原因</span>
        </div>
      ...
      <div class="my_spans" style="border:none">
        <div class="flex_item">
          <img
            src="../../../assets/images/icon_why2.png"
            alt=""
        ...
        <div>
          <input type="text" class="fr_data" readonly />
        </div>
      </div>
      <van-field
        name="fileList"
    ...
    <van-popup v-model="alterTeacherPicker" position="bottom" class="alter-pop">
      <alterTeacher @closePop="closePop" :isRadio="false"></alterTeacher>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    closePop(res, id, name) {
      this.alterTeacherPicker = res;
      if (!name) {
        this.valuess = "";
      } else {
        this.valuess = name;
    ...
    startTap() {
      if (this.value1 == "") {
        this.$toast.fail("请选择变更周次");
      } else if (this.value3 == "") {
        this.$toast.fail("请选择星期");
      } else {
    ...
    async getWeek() {
      ...
      let range = this.$route.query.oldWeek.match(/\d?(.\d+)?/gi);
...
.teacher111 {
  /deep/.alter-pop.van-popup.van-popup--bottom {
    max-height: 100% !important;
    min-height: 100% !important;
  }
}
/* 审核人 */
.teacher111 .close-icon {
  background: url(../../../../static/img/close.png) no-repeat center;
}
.teacher111 .confirm-icon {
  background: url(../../../../static/img/confirm.png) no-repeat center;
}
.teacher111 .pop-title i {
  width: 22px;
  height: 22px;
  background-size: 22px 22px;
  padding: 10px;
}
.teacher111 .pop-title {
  display: flex;
  align-items: center;
...
.teacher111 .person button {
  border-radius: 3px;
  padding: 7px 15px;
  margin-right: 12px;
  margin-bottom: 10px;
}
.teacher111 .apply-pop .person .per-btn {
  background-color: #ecedfd;
  color: #7469f8;
}
.teacher111 .person .cur {
  background-color: #7469f8;
  color: #ecedfd;
}
.teacher111 .van-popup.van-popup--bottom .van-picker .van-picker-column {
  position: relative;
}
.teacher111
...
.cho input::-webkit-input-placeholder,
.cho textarea::-webkit-input-placeholder {
  text-align: right;
  color: #dfe0e6;
  font-size: 16px;
}
