<template>
  <div>
    <Header
      title="调课审核"
      type="0"
      :route="this.back ? this.back+'?path=/pass/index' : '/teacher/examine/adjustList'"
      :casLogin="casLogin" 
    ></Header>
    <div class="center">
      <div class="center-list">
        ...
        <div class="list-left">
          ...
          <div class="left-line">
            <div style="width:40%">
              <span>教师:</span>
              <span style="margin-left: 5px;">{{ route_tk.skls }}</span>
            </div>
      ...
      <div class="center-content-gray">
        ...
        <div class="gray-line-one">
          <div class="gray-left" style="text-decoration: line-through;">
            原时间
          </div>
          <div
            class="gray-right"
      ...
      <div class="center-content-white" style="align-items: flex-start;">
        <div class="white-left">附件</div>
        <div
          class="white-right"
          style="width: 60%;word-break: break-all;text-align: right"
        >
          <div id="myMain" @click="reImage"></div>
        </div>
      </div>
      <!-- <van-field
            name="uploader"
    ...
    <van-popup v-model="show" id="reImg"></van-popup>
    <div class="bottom">
      <van-button class="bottom-button" type="default" @click="pushExamine"
        >审核</van-button
      >
    </div>
  </div>
...
export default {
  ...
  mounted() {
    this.getList();
    this.back=JSON.parse(decodeURIComponent(this.$route.params.route_params)).route
  },
  methods: {
    reImage() {
      this.show = true;
      this.$nextTick(() => {
        if (this.img.width >= 320) {
          this.img.style.width = "320px";
        } else {
          this.img.style.width = "auto";
        }
        document.getElementById("reImg").appendChild(this.img);
      });
    },
    async getList() {
      this.route_tk = JSON.parse(decodeURI(this.$route.params.route_params));
      console.log(this.route_tk);

      const ret = await Api.downfile({
      ...
      if (ret.code != 0) {
        if (ret.type == "application/json") {
          ...
          span.innerText = "无";
          document.getElementById("myMain").appendChild(span);
          return;
        }
        ...
        this.img1.style.width = "80px";
        this.img1.style.height = "80px";
        document.getElementById("myMain").appendChild(this.img1);
      }
    },
    pushExamine() {
      ...
      this.$router.push({
        name: "examineCommom",
        params: {
          route: "adjustListDetail,1",
          route_params: encodeURI(JSON.stringify(this.route_tk))
        }
      });
    }
  }
