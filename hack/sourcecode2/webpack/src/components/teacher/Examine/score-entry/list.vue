<template>
  <div>
    <Header
      title="成绩录入审核"
      type="0"
      route="/teacher/examine/searchScore"
      :casLogin="casLogin"
    ></Header>
    <div class="center">
      <div class="search-top">
        <div class="top-line" v-for="(item, index) in search_list" :key="index">
          {{ item.name }}
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    pushDetail(item) {
      item.sh_flag = "008"
      item.route = "/teacher/examine/searchScore";
      this.$router.push({
