<template>
  <div>
    <Header v-if="schoolCode=='10013'"  title="教室借用审核"></Header>
    <Header v-else title="教室借用审核" type="0" :route="back?back:'/teacher/examineIndex'" :casLogin="casLogin"></Header>
    <div class="center">
...
export default {
  ...
  created(){
    this.back=this.$route.query.path
  },
  methods: {
    async getList() {
    ...
    pushDetail(item) {
      item.route=this.$route.query.path
