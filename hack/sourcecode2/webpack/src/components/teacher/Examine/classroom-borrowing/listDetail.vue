<template>
  <div>
    <Header
      ...
      type="0"
      :route="
        route_tk.route ? route_tk.route : '/teacher/examine/borrowingList'
      "
      :casLogin="casLogin"
    ></Header>
    <div class="center">
    ...
    <div class="bottom-btn">
      <button @click="pushExamine">审核</button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    this.routeList = JSON.parse(
      decodeURIComponent(this.$route.params.route_params)
    );
  },
  methods: {
    async getList() {
