<template>
  <div>
    ...
    <div class="stop-item">
      ...
      <div v-if="isShowFile">
        <van-button class="bottom-button" type="default" @click="CheckFile"
          >查看附件</van-button
        >
        <van-button class="bottom-button1" type="default" @click="pushExamine"
          >审核</van-button
        >
      </div>
      <div v-else>
        <van-button class="bottom-button2" type="default" @click="pushExamine"
          >审核</van-button
        >
      </div>
...
export default {
  data() {
    return {
      ...
      columns1: [{ text: "通过", value: "1" }, { text: "不通过", value: "0" }],
      showPicker1: false,
      peoplePop: false,
      shr_list: [],
      uploader: [],
  ...
  methods: {
    ...
    async pushExamine() {
      ...
      if (res.code == "1") {
        this.$router.push({ path: "/teacher/examine/examineCommomSuccess",query:{params:this.route_tk} });
      } else {
        this.$toast(res.Msg);
        setTimeout(() => {
