<template>
  <div class="attend-cal">
    <Header title="考勤" :casLogin="casLogin"></Header>
  ...
  methods:{
    ...
    calcClass(k,d){
      if(k == 1 && d == 1){
        ...
        return 'note-mark p-active'
      }else if(k == 0 && d == 0){
        return 'p-active'
      }
    },
    async loadCalendar(e) {
      let curDate = new Date().getFullYear() + '-' + (new Date().getMonth() + 1)
      const params = {
        'yf':e ? e : curDate
      }
      ...
      // 初始化当前日期课程
      if(!e){
        let cur = res.data.filter(item => item.isdqrq == '0')[0]
        this.getTimetable(cur.zc,cur.xqid,cur.isdqrq)
      }
    },
    beforeCal(e) {
      this.noneList = []
...
<style lang="less" scoped>
  ...
  .calendar-main>h4{color: @attendance-color-1;font-size: 18px;margin: 10px;display: flex;align-items: center;width: fit-content;}
  .cal-body>ul{display: flex;color: @attendance-color-2;}
  .cal-body>ul>li{width: 14%;text-align: center;}
  .cal-body>div>ul{display: flex;flex-wrap: wrap;margin-top: 10px;}
  .cal-body>div>ul>li{margin-bottom: 5px; width: 14%;height: 35px;line-height: 33px;color: @attendance-color-1;font-weight: bold;display: flex;align-items: center;justify-content: center;text-align: center;;}
  .note-mark{border-radius: 50%;width: 35px;position: relative;}
  .note-mark.p-active{
  ...
  .note-mark.p-active::after,.have-class.p-active::after {
    position: absolute;
    width: 6px;
    height: 6px;
    border-radius: 50%;
  ...
  .cal-foot{display: flex;align-items: center;color: @attendance-color-1;font-size: 14px;margin: 10px 10px 0px;}
  ...
  .cal-item>p{width: 10%;line-height: 78px;text-align: center;color: @attendance-color-2;font-weight: 600;}
  .item-info{color: #ffffff;height: 61px;border-radius: 5px 0 0 5px;padding: 8px 20px;box-sizing: border-box;position: relative;}
  .item-info1{color: @attendance-color-2;height: 61px;border-radius: 5px 0 0 5px;padding: 8px 20px;box-sizing: border-box;background-color: #F6F6F6 !important;}
  ...
  .item-info1>p{color: @attendance-color-1;}
  ...
  .item-info1>p::before{background-color: @attendance-color-1;}
  ...
  .attend-cal .van-ellipsis.van-picker-column__item.van-picker-column__item--selected{color: @attendance-color-1;font-weight: 600;}
  .hidden{
    width: 100%;
  }
  .hidden.active{
  ...
  .circle-rate .van-circle__text{font-size: 10px;color: @attendance-color-1;font-weight: bold;}
