<template>
  <div class="attendance-dtl">
    ...
    <div class="detail">
      ...
      <p>
        <i class="icon1"></i><span>教师：</span><b>{{ detailList.jsxm }}</b>
      </p>
      <p><i class="icon2"></i><span>教室：</span>{{ detailList.jsmc }}</p>
      <p>
        <i class="icon3"></i><span style="min-width:80px;">本课时间：</span
        >{{ detailList.jc }}节，{{ detailList.kssj }} - {{ detailList.jssj }}
      </p>
        ...
        <div>
          <el-table :data="attendanceList" stripe>
            ...
            <el-table-column prop="studentNumber" label="学号" sortable>
              <template slot-scope="scope">
                <span style="color: #212F6E;">{{
                  scope.row.studentNumber
                }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="attendanceResult"
              label="状态"
              sortable
                ...
                >
                  <span
                    :style="
                      'font-weight:600;' +
                        'color:' +
                        clacResult(scope.row.attendanceResult)
                    "
                    >{{
                      scope.row.attendanceResult == ""
                        ? "未签到"
                        : scope.row.attendanceResult
                    }}</span
                  >
  ...
  methods: {
    ...
    getQrCode() {
      let qrcode = new QRCode("qrcode", {
        ...
        text:
          this.qrcodeUrl +
          "/index.html#/student/attendance?course=" +
          this.courseList +
          "&week=" +
          this.week +
          "&isdqrq=" +
          this.$route.query.isdqrq,
        // colorDark : "#000000",
        // colorLight : "#ffffff",
...
.attendance-dtl .van-picker__cancel {
  color: rgba(0, 0, 0, 0);
...
.attendance-dtl .van-picker__confirm {
  color: rgba(0, 0, 0, 0);
